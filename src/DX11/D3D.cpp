...
dx11::D3D::D3D(int screenWidth, int screenHeight, HWND hwnd, bool fullscreen, float screenDepth, float screenNear)
{
...
void dx11::D3D::createDevice()
{
	...
	SIZE_T MaxSize = 0;
	for (size_t ap = 0; ap < vAdapters.size(); ap++)
	{
		...
		if ((desc.DedicatedVideoMemory > MaxSize) && (D3D11CreateDevice(pAdapter, D3D_DRIVER_TYPE_UNKNOWN, nullptr, creationFlags, featureLevels, 2, D3D11_SDK_VERSION, &device, NULL, &deviceContext) == S_OK))
		{
			//pAdapter->GetDesc1(&desc);
...
void dx11::D3D::createRenderTargetView()
{
	ID3D11Texture2D* backBufferPtr;
	// Configure back buffer
	swapChain->GetBuffer(0, __uuidof(ID3D11Texture2D), (LPVOID*)&backBufferPtr);
	device->CreateRenderTargetView(backBufferPtr, nullptr, &renderTargetView);
	backBufferPtr->Release();
	backBufferPtr = nullptr;
}

// Creates a default depth buffer
void dx11::D3D::createDepthBuffer()
{
...
void dx11::D3D::createStencilBuffer()
{
...
void dx11::D3D::createDefaultRasterState()
{
	...
	projectionMatrix = DirectX::XMMatrixPerspectiveFovLH(fieldOfView, screenAspect, nearPlane, farPlane);

	// Initialize the world matrix to the identity matrix.
	worldMatrix = DirectX::XMMatrixIdentity();

	// Create an orthographic projection matrix for 2D rendering.
	orthoMatrix = DirectX::XMMatrixOrthographicLH((float)screenwidth, (float)screenheight, nearPlane, farPlane);
}

// Creates additional raster state, in this case for depth disabled 2D rendering.
void dx11::D3D::createDepthlDisableState()
{
...
void dx11::D3D::createBlendState()
{
...
void dx11::D3D::beginScene(float red, float green, float blue, float alpha)
{
	float color[4];
...
void dx11::D3D::endScene(bool vsync)
{
	if (vsync)
	{
...
// Get 3D device
ID3D11Device* dx11::D3D::getDevice()
{
	return device;
}

ID3D11DeviceContext* dx11::D3D::getDeviceContext()
{
	return deviceContext;
}

DirectX::XMMATRIX dx11::D3D::getProjectionMatrix()
{
	return projectionMatrix;
}

DirectX::XMMATRIX dx11::D3D::getWorldMatrix()
{
	return worldMatrix;
}

DirectX::XMMATRIX dx11::D3D::getOrthoMatrix()
{
	return orthoMatrix;
}

// Enable/disable the ZBuffer. Uses previously created depth states.
void dx11::D3D::setZBuffer(bool b)
{
	zbufferState = b;
	if (zbufferState)
	{
...
bool dx11::D3D::getZBufferState()
{
	return zbufferState;
}

// Sets the blending state, to enable/disable alphablending
void dx11::D3D::setAlphaBlending(bool b)
{
	alphaBlendState = b;
...
bool dx11::D3D::getAlphaBlendingState()
{
	return alphaBlendState;
}

// Set the back buffer as the render target
void dx11::D3D::setBackBufferRenderTarget()
{
	deviceContext->OMSetRenderTargets(1, &renderTargetView, depthStencilView);
	return;
}
...
void dx11::D3D::setWireframeMode(bool b)
{
	wireframeState = b;
	if (wireframeState)
	{
...
bool dx11::D3D::getWireframeState()
{
