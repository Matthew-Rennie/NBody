...
project  "Sandbox"
  kind "ConsoleApp"
  language "C++"
  targetname "Sandbox" -- the name of the executable saved to targetdir
  ...
  local output_dir_root         = ROOT .. "/bin/bin/bin_%{cfg.platform}_%{cfg.buildcfg}"
  targetdir(output_dir_root)    -- Where all output files are stored
  debugdir(output_dir_root)     -- working directory when debugging
  ...
  postbuildcommands 
  {
    "if not exist ..\\bin\\bin\\bin_%{cfg.platform}_%{cfg.buildcfg}\\res mkdir ..\\bin\\bin\\bin_%{cfg.platform}_%{cfg.buildcfg}\\res", -- create dir if it doesnt exist
    "xcopy /s /y ..\\res ..\\bin\\bin\\bin_%{cfg.platform}_%{cfg.buildcfg}\\res" -- copy contents of /res/ to outDir
  }
